...
that as a tampered app and reports **"wispr-fox is damaged and can't be opened"** —
...
2. **Keep `hardenedRuntime: false`** unless you are wiring up notarization. It
   defaults to **true** in Tauri 2, and hardened runtime denies **microphone access**
   regardless of what the user grants in System Settings. `src-tauri/entitlements.plist` already carries
   `com.apple.security.device.audio-input` for when that day comes — enabling
   hardened runtime without it silently kills dictation while the app still launches.
...
4. **Do not delete the "Verify macOS bundle signature" step** in
   ...
   before users do.
5. Ad-hoc signing does **not** make the Accessibility grant persist across
   updates — the hash changes every build. `docs/MACOS_SIGNING.md` covers the
   stable self-signed certificate that fixes that separately.
...
```bash
codesign --verify --deep --strict --verbose=2 /Applications/wispr-fox.app
```
